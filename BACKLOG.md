...
- **US-7.1** Tournoi de type « équipe » : taille d'équipe et capacité comptée en équipes — **S** — dép. EPIC-1 (livré). *(ex-7.1)* — ✅ **Livrée (2026-08-27)**
  > Migration 0041 : `tournaments.team_size` (2 à 8) sous **contrainte croisée avec le type** — un tournoi par équipes sans taille n'est pas un brouillon incomplet, c'est un tournoi impossible à apparier. Le type « Équipe » du formulaire, jusque-là grisé derrière un badge « Bientôt », est ouvert. La capacité **change d'unité** avec le type (équipes, plus joueurs) : libellé, bornes, pas et message d'erreur suivent dans les deux applications, et le formulaire affiche le total dérivé.
  > **Effet de bord traité** : `duplicate_tournament` (0026) recopiait `type` sans `team_size` — la duplication d'un tournoi par équipes aurait échoué sur la contrainte, avec une erreur incompréhensible. Leçon générale : toute fonction qui recopie une ligne colonne par colonne est à rouvrir à chaque ajout de colonne.
  > Le badge de format dit « Par équipes de 3 » et non « Équipe » : la taille est ce qui décide si je peux venir.
  > *Divergence mineure avec la spec PO* : un simple pas-à-pas 2→8 au lieu de raccourcis 3 / 5 / libre. Un contrôle au lieu de trois, pour un champ rempli une fois.
- **US-7.2** Inscription d'équipe en base : `team_registrations`, rattachement des joueurs, capacité et liste d'attente — **M** — dép. US-7.1, EPIC-4. *(ex-7.2, moitié base)* — ✅ **Livrée (2026-08-27)**
  > Migration 0042. `team_registrations` se pose **au-dessus** de `registrations` : chaque joueur du roster reste une ligne ordinaire, avec `team_registration_id` et `roster_position`. Fonctions `register_team`, `withdraw_team`, `update_team_roster`, `promote_waitlist_team`, `team_slots_taken`.
  > **Une équipe entre ou attend en entier** — jamais deux joueurs inscrits et un troisième en liste d'attente. Verrou `for update` sur le tournoi, comme la course à la dernière place de la 0004.
  > **Les refus nomment la personne** : `NOT_A_MEMBER:Morvath`, `PLAYER_ALREADY_REGISTERED:Nyssa`. Un capitaine ne peut désinscrire personne : le message doit dire qui doit agir, pas envoyer chercher lequel à la main.
  > La **règle permanente de la 0038** s'applique d'elle-même : `anon` ayant des droits colonne par colonne sur `registrations`, les deux colonnes ajoutées lui restent invisibles sans rien faire. Vérifié par assertion.
  > Trigger `tournaments_team_size_guard` : la taille d'équipe se fige dès la première inscription — la changer sous les pieds d'équipes déjà inscrites produirait des rosters de la mauvaise taille sans que rien ne le signale.
...
- **US-7.3** Le capitaine inscrit son équipe depuis l'app mobile — **M** — dép. US-7.2. **Avis `ux-ui` rendu.** *(ex-7.2, moitié écran)* — ✅ **Livrée (2026-08-27)**
  ...
  > **Migration 0043** — le trou que l'écran a révélé : un joueur du roster est inscrit **sans l'avoir demandé**, il doit pouvoir sortir, et lui seul le peut. `withdraw_from_tournament` le retirait mais laissait `team_registration_id` en place : l'équipe **paraissait complète alors qu'elle ne l'était plus**, et on l'aurait découvert le jour J devant une table vide. Elle détache désormais du roster, sans promouvoir personne — la place appartient toujours à l'équipe, qui n'a perdu qu'un joueur. Ajouté au passage : un garde-fou refusant toute désinscription après le lancement (le client ne l'offrait déjà pas ; la règle vit maintenant en base), et `team_roster_gaps()` pour voir les rosters incomplets **avant** la première ronde.
- **US-7.4** Pointage des équipes le jour J — **S** — dép. US-7.3, EPIC-3. — ✅ **Livrée (2026-08-27)**
  > Migration 0044 : `set_team_check_in` appelle le pointage individuel **en lot** — la brique de la 0007 n'est pas réécrite. Une équipe se présente à l'accueil ensemble, l'organisateur fait un geste pour elle entière.
  > `team_check_in_state()` fait remonter en une requête ce qui se découvrirait sinon **devant une table vide** : les rosters amputés d'un joueur parti, et les factions non déclarées.
  > **Règle qui surprendra, et hypothèse à confirmer** : une équipe ne peut être pointée que si tous ses joueurs ont déclaré leur faction. Ce n'est pas de la rigueur administrative — dans un tournoi par équipes le capitaine adverse apparie **en regardant les factions**, sans elles le format perd son sens. Le refus les nomme, et l'organisateur comble en trente secondes depuis Inscrits (0040). Si la règle gêne le jour J, la lever tient en une migration : le blocage vit en base, pas dans l'écran.
  > Back office : `components/team-check-in.tsx` remplace la liste de joueurs quand le tournoi est de type équipe, en réutilisant les classes CSS existantes — le back office n'a aucune bibliothèque de composants, chaque classe nouvelle est du CSS de plus à tenir. Aucun rechargement après pointage.
  > 5 assertions SQL passées, dont : tiers refusé, blocage nominatif sur faction manquante, pointage qui emporte le roster entier, dépointage symétrique.
- **US-7.5** Rondes suisses entre équipes — **L** — dép. US-7.4. *(ex-7.3)* — ✅ **Livrée (2026-08-27)**
  ...
  > Migration 0045 : table `team_pairings` (la **rencontre**), colonne `pairings.team_pairing_id`, `already_met_team` / `swiss_pair_teams` (**duplication assumée** des fonctions de la 0011 : les généraliser aurait touché le chemin critique du jour J déjà éprouvé sur les tournois individuels), `start_team_tournament`, `generate_next_team_round`, `seed_team_tables`, la vue `team_encounter_results` et `team_standings`.
  > **Convention qui porte tout le reste** : dans un match de rencontre, `player_a` appartient toujours à l'équipe A. Le score d'équipe se lit alors en sommant `score_a` d'un côté et `score_b` de l'autre — sans jointure de plus, et sans pouvoir inverser deux équipes.
  > **L'aiguillage vit en base** : `start_tournament` et `generate_next_round` délèguent selon `tournaments.type`. Le back office continue de les appeler sans savoir qu'il existe des tournois par équipes — une règle métier écrite dans le client serait une règle qui n'existe que dans une des deux applications.
  > La rencontre k occupe les tables (k−1)×N+1 à k×N : les joueurs d'une même rencontre sont côte à côte dans la salle, ce que les organisateurs font déjà à la main. Un roster incomplet produit une table en **forfait 15-5** plutôt qu'une table absente : elle se voit.
  > **Rien n'est stocké du score d'équipe** — tout se dérive de `pairings`. Corriger le score d'une table met à jour la rencontre et le classement sans recalcul, et sans divergence possible. Une rencontre n'entre au classement que **toutes ses tables saisies** : annoncer un vainqueur sur deux tables sur trois serait raconter une histoire fausse.
  > **6 assertions SQL passées** sur un tournoi complet à 4 équipes de 3 : lancement (2 rencontres, 6 tables numérotées), rattachement de chaque table à sa rencontre et **côté correct pour chaque joueur**, saisie des six scores par la fonction existante, classement d'équipe cohérent (60 points au premier), ronde 2 sans rencontre rejouée, et **classement individuel fonctionnel sans une ligne de code spécifique** — la preuve que le modèle tient.
  > Ajouté au passage : le lancement du tournoi depuis l'écran de pointage par équipe, avec le décompte des rencontres et le nom des équipes qui seront écartées. Sans lui, un tournoi par équipes ne pouvait pas démarrer.
- **US-7.6** Score de rencontre et classement des équipes — **M** — dép. US-7.5. *(ex-7.5, moitié classement)* — ✅ **Livrée (2026-08-27)**
  > `team_standings` était déjà posée par la 0045 (l'appariement suisse en dépend). Cette US livre les écrans : **un seul écran de classement, un segmenté Équipes / Joueurs**, dans les deux applications. Deux écrans jumeaux se confondraient dans la navigation et doubleraient la modale des départages, pour répondre à la même question — « où j'en suis ».
  > Mobile : la **ligne de mon équipe** se met en évidence comme ma ligne de joueur, c'est la même question posée à une autre échelle. Le choix d'onglet ne se mémorise pas — un onglet consulté trente secondes n'a pas de préférence à retenir.
  > **Les deux classements existent**, et c'est un choix : effacer le classement individuel serait injuste envers qui fait cinq victoires sur cinq dans une équipe médiocre. La légende du tableau change avec l'onglet — « V : victoires » désignerait sinon des victoires de joueur absentes du tableau des équipes.
  > Vide traité explicitement : tant qu'aucune rencontre n'est **entièrement** saisie, le classement des équipes est vide et le dit, avec un renvoi vers l'onglet Joueurs.
...
- **US-7.7** Écran d'appariement capitaines — **L** — dép. US-7.5, US-7.6. **Avis `ux-ui` rendu.** *(ex-7.4)* — ✅ **Livrée (2026-08-27)**
  > Migration 0046 : table `captain_picks` et six fonctions. Protocole **« pose – deux – choix »** itéré N−1 fois, puis dernier match automatique — demander un geste pour une non-décision ferait perdre du temps à quatre personnes debout.
  > **Le journal EST l'état.** `captain_picks` n'est pas une trace posée à côté d'une machine à états : c'est d'elle que `team_pairing_state()` dérive à qui est le tour et quel geste est attendu. Une seule source, rien à resynchroniser, et l'appariement reste explicable geste par geste après coup.
  > **La décision « pas de temps réel » n'est pas rouverte**, et c'est le protocole qui le permet : strictement séquentiel et **sans aucun secret**, chacun voit à tout instant les deux rosters, les matchs figés et à qui est le tour. On n'attend jamais un geste invisible. Bouton « Actualiser » plein format en plus du tirer-pour-rafraîchir : debout, ce dernier ne se découvre pas.
  > **Aucune minuterie** : elle exigerait un cron, une horloge partagée et du temps réel, pour un problème que l'organisateur règle en marchant trois mètres. À la place, deux gestes en back office — « Confier aux capitaines » (défait les tables composées dans l'ordre des rosters, refusé si un score existe déjà) et « Compléter » (termine à leur place). Les actions de l'organisateur sont tracées `acted_by_organizer`.
  > **11 assertions SQL passées** : négociation complète à 3 joueurs jusqu'au verrouillage, dernier match auto-formé, 6 gestes journalisés pour 2 passes, convention « player_a est de l'équipe A » préservée, et cinq refus — non-capitaine, hors de son tour, ouverture par un capitaine, présenter un seul joueur, poser deux fois de suite.
...
- **US-7.8** Le tournoi par équipes vécu côté joueur (bloc « Le jour J », écran des tables groupé par rencontre) — **M** — dép. US-7.6. **Avis `ux-ui` rendu.** — ✅ **Livrée (2026-08-27)**
  > Le bloc « Le jour J » **garde sa hiérarchie** : la rencontre encadre le match, elle ne le remplace pas. Le nom des deux équipes au-dessus, le score de rencontre en dessous, et entre les deux le numéro de table en 64 px et l'adversaire — ce qu'on vient chercher en marchant.
...
- **Corrigé au premier lancement réel (2026-09-11)** — deux défauts trouvés par le porteur en lançant la Coupe des Provinces, tous deux dans `launch-tournament-modal.tsx` :
  > 1. **La modale n'était pas au courant des tournois par équipes.** Elle annonçait « 12 joueurs pointés présents seront appariés au hasard sur 6 tables » là où ce sont **4 équipes** qui s'apparient en **2 rencontres**. Elle décrivait un tirage qui n'a jamais lieu : en tournoi par équipes, les tables partent de l'ordre des rosters puis se négocient entre capitaines. La modale lit désormais le type et va chercher elle-même `team_check_in_state`, la page appelante ne comptant que des joueurs.
  > 2. **Le message d'erreur accusait le réseau d'une réussite.** Le premier clic avait lancé le tournoi ; le second s'est heurté au verrou (« Ce tournoi ne peut plus être lancé ») et l'écran affichait « Vérifiez votre connexion et réessayez » — la panne imaginaire au lieu du fait. Ce cas dit maintenant que le tournoi **est déjà lancé** et propose le seul geste utile, recharger. Les autres refus affichent le **message exact de la base**, qui est déjà une phrase lisible (« Il faut au moins deux équipes présentes… ») : la remplacer par une phrase générique envoyait chercher une panne là où la base avait dit ce qui manquait.

- **US-7.11** Le roster de tournoi se remplit **à deux mains** — **M** — dép. US-7.2, US-7.3. — ✅ **Livrée (2026-09-04)**, migration 0053.
  ...
  >
  > **Notes de livraison (2026-09-04).** Migration 0053 : `register_team` et `update_team_roster` acceptent un roster de 0 à `team_size` ; `join_team_roster` (le volontaire), `remove_from_roster` (le capitaine défait), `invite_to_roster` (l'appel) et `my_tournament_roster` (l'état lu par les deux écrans) s'y ajoutent.
  > **L'invitation n'est pas un objet** : ni table, ni état, ni expiration — une notification, et le membre prend la place lui-même par le chemin ordinaire. Une table `roster_invitations` aurait apporté un cycle de vie complet (acceptée, refusée, périmée, révoquée) pour un service que le bouton rend déjà, et deux chemins d'entrée qui auraient fini par diverger.
  > **Le modèle n'a rien eu à inventer en aval** : `seed_team_tables` (0045) produisait déjà un forfait 15-5 sur une place vide, et `set_team_check_in` (0044) ne refuse que les factions manquantes. Le roster partiel était donc déjà survivable de bout en bout — seule l'entrée était fermée.
  > **La question laissée ouverte est tranchée par défaut** : la place libre **la plus basse** d'abord, donc l'ordre d'arrivée. Le capitaine garde la main puisqu'il peut retirer puis recomposer. Un réordonnancement explicite, s'il est voulu, vivra dans une migration suivante sans toucher à ces fonctions.
  > Mobile : `use-tournament-roster.ts` + `roster-card.tsx` (les places vides s'affichent — une place qu'on ne montre pas ne se prend pas), écran du capitaine assoupli, et le message « seul le capitaine inscrit l'équipe » corrigé : il était devenu faux.
  > `send-push` **redéployée en v6** avec la branche `roster_invite` : sans elle, un `kind` inconnu retombe sur `return []` et l'événement est supprimé — l'invitation serait partie à la poubelle en silence.
  > **Corrigé au premier essai du porteur (2026-09-04)** : le bouton « Appeler » ne montrait rien. L'appel partait bien — la file le prouve — mais **inviter ne change rien au roster**, donc l'écran restait identique et le bouton paraissait mort. Deux manques, pas un : aucune confirmation, et le hook ne vidait pas la file après l'action, contrairement à la convention du projet — l'invitation attendait qu'une autre action de l'app la pousse. Le bouton passe désormais à « Appelé », une ligne verte nomme le joueur, et `flushPushQueue()` part aussitôt. L'état « appelé » vit dans l'écran, **pas en base** : la file se vide, et un état qui disparaît au vidage mentirait plus qu'il n'informerait.
  > **10 assertions SQL passées** puis annulées : engagement à roster vide, état initial, invitation unique malgré deux appels, volontaire en position 1, doublon refusé, roster plein refusé, invitation sur roster plein refusée, retrait puis reprise **exacte de la place libérée**, dépassement refusé, joueur sans équipe engagée refusé. Plus la lecture de `my_tournament_roster` sous quatre points de vue, dont `anon` qui reçoit « permission denied ».
...
- **US-9.2** Statistiques par faction jouée (parties, victoires) — **M** — dép. US-9.1. — ✅ **Livrée (2026-08-22)**, précédée d'un prérequis non prévu
  > **L'US ne pouvait pas être livrée telle quelle**, et c'est l'agent `ux-ui` qui l'a établi : `army_lists.faction` était un **champ texte libre**. Agréger dessus aurait fragmenté silencieusement — « Nighthaunt », « nighthaunt », « NH » comptés comme trois armées. **Exactement le piège des régions**, déjà payé et documenté en tête de `src/lib/regions.ts`. Pire, le champ était **prérempli avec `profiles.faction_favorite`** : la donnée était déjà, pour une part inconnue, la faction favorite déguisée.
  > **Prérequis livré d'abord** : `src/lib/factions.ts` (24 factions AoS groupées par Grande Alliance, `matchFaction()` qui ramène une saisie libre vers l'entrée officielle et renvoie `null` plutôt que de deviner) et `src/components/faction-picker.tsx`, branché sur la soumission de liste **et** sur le profil — deux saisies libres pour la même notion, c'était deux vocabulaires. **Liste validée par le porteur le 27 août 2026** : Beasts of Chaos retirée (passée en Legends, elle ne s'aligne plus en tournoi), Helsmiths of Hashut ajoutée, et **pas d'entrée « Autre »** — un fourre-tout se remplit toujours, et une ligne « Autre » ne dit rien de personne dans une statistique. Une armée manquante doit donc se voir comme un manque à combler.
  > Migration 0037 : la faction devient une **colonne de `player_history`**, et les deux fonctions dédiées de la 0036 sont supprimées. Trois appels réseau pour une même page, c'était trois états de chargement et trois façons d'échouer.
  > Affichage : bloc « Factions jouées » en pied de `/historique`, `src/components/faction-row.tsx`. **Que des entiers — aucun pourcentage, aucun superlatif, tri par parties jouées.** Sur cinq parties, un « 60 % de victoires » a un intervalle réel d'environ 15 % à 95 % : il n'informe pas, il fait croire à une mesure. Le taux de victoire est renvoyé à l'US-11.3 (stats méta), où l'échantillon est celui de la communauté.
  > La ligne **« Faction non renseignée »** n'est pas un aveu de faiblesse : c'est elle qui fait boucler les totaux avec les tuiles du haut, donc qui rend le bloc crédible. Le texte dit la cause sans reprocher — le champ étant obligatoire à la soumission, l'absence vient du tournoi qui n'a pas demandé de liste, pas d'un oubli du joueur.
  > 6 assertions SQL passées, dont l'étanchéité : un tiers voit l'historique d'un joueur mais **jamais ses factions** (RLS de la 0018).
  > **Proposition de l'agent `ux-ui`, arbitrée le 27 août 2026** : elle est devenue l'US-9.3 ci-dessous.
- **US-9.3** Déclarer sa faction pour un tournoi, sans liste d'armée — **M** — dép. US-9.2 (livrée), US-2.3 (livrée). — ✅ **Livrée (2026-08-27)** en trois incréments
  > **Pourquoi cette US existe :** la section « Factions jouées » (US-9.2) n'est alimentée que par `army_lists.faction`. Or beaucoup de tournois ne demandent pas de liste : pour ceux-là la faction n'est jamais connue, et « Faction non renseignée » domine un bloc censé raconter la carrière du joueur.
  > **Effet de bord attendu, aussi important que l'objectif :** l'app affiche aujourd'hui `faction_favorite` sous chaque pseudo (inscrits, tables, classement, back office). **Une préférence de profil y tient lieu d'armée alignée** — l'histoire fausse que la décision 14 interdit partout ailleurs. L'US installe la donnée juste et met fin à la substitution.

  **En tant que** joueur inscrit, **je veux** déclarer en deux taps la faction que j'aligne à ce tournoi **afin de** ne pas dépendre d'une liste d'armée pour que mon historique soit juste.

  1. **Migration** : colonne `faction` sur `public.registrations` (texte nullable, valeur de la liste fermée de `src/lib/factions.ts`), écrite par `set_registration_faction(p_tournament_id, p_faction)` en `security definer` (`grant` à `authenticated`, `revoke` de `anon`), qui refuse un appelant non inscrit et un tournoi dont le statut n'est pas `open`.
  2. **Confidentialité** — arbitrée par le porteur le 27 août : la faction déclarée est **visible des membres connectés, jamais des visiteurs anonymes**, soit le niveau où se trouve déjà la faction favorite. Le secret qui compte en AoS est le *contenu* de la liste, pas le nom de l'armée — c'est bien ce que protège la 0018. `registrations` étant lisible par `anon` et la RLS ne filtrant jamais les colonnes, la migration applique le patron de la 0016 : `revoke select` de `anon`, puis `grant select` colonne par colonne sur les seules colonnes existantes. **Règle permanente qui en découle : toute future colonne de `registrations` est privée par défaut.**
  3. **« Combler oui, réécrire non »** — arbitré par le porteur le 27 août. Tant que le tournoi est `open`, on déclare et on modifie librement. Une fois lancé, on peut encore renseigner une faction **absente** — combler un trou n'a jamais falsifié personne, et c'est justement après coup que le joueur découvre le manque dans ses statistiques — mais plus **changer** celle qui a déjà été annoncée aux adversaires. Garanti en base par un trigger `before update`, pas seulement par l'écran : une règle qui ne vit que dans l'interface n'est pas une règle, c'est une politesse.
  4. **Source unique de vérité** : `player_history` (nouvelle migration, `drop function` préalable — piège 13) lit `registrations.faction` et non plus `army_lists.faction`. Reprise des factions des listes existantes vers les inscriptions. `submit_army_list` écrit désormais les deux dans la même transaction : **un seul champ dans la tête du joueur, deux portes pour l'atteindre.**
  5. **Pas de carte de plus** : la fiche empile déjà 6 à 8 blocs, et une 9e carte pour un champ facultatif serait le pire rapport surface/valeur de l'écran. `army-list-card.tsx` devient `preparation-card.tsx` — carte **« Ma préparation »** à deux sous-blocs séparés d'un filet : *Faction* (une ligne, un tap) puis *Ma liste d'armée* (inchangé). Les deux répondent à la même question du joueur — « qu'ai-je à faire avant le jour J ? » — et n'existent jamais l'une sans l'autre. Trois états de faction : non déclarée, déclarée (valeur + tap pour changer), figée. Réutilise `FactionPicker` — aucun nouveau sélecteur. Enregistrement **immédiat au choix**, sans bouton « Enregistrer » ; en cas d'échec, retour à la valeur précédente et lien « Réessayer » qui **rejoue la faction mémorisée en variable** (piège n° 3 : l'action différée reçoit sa cible).
  6. **Aucun préremplissage silencieux depuis `faction_favorite`** : la carte propose un raccourci explicite (« Tu joues *Nighthaunt* ? Oui · Choisir une autre ») ; rien ne s'enregistre sans un tap. Leçon de l'US-9.2, où la faction des listes était déjà, pour une part inconnue, la faction favorite déguisée.
  7. **Gel** : voir le critère 3. Une exception s'y ajoute — si une liste d'armée a été **validée**, la faction est figée avec elle quel que soit le statut du tournoi, et la carte le dit : l'organisation a engagé sa parole dessus.
  8. **Fin de la substitution** : partout où `faction_favorite` s'affiche dans un contexte de tournoi (`player-row.tsx`, `tables.tsx`, `classement.tsx`, `jour-j-card.tsx`, et côté back office `inscrits.tsx`, `check-in.tsx`, `classement.tsx`, `rondes.tsx`), on affiche la faction **déclarée** ; à défaut, rien.
  9. **Historique** : le libellé « Faction non renseignée » est révisé — sa cause n'est plus « le tournoi n'a pas demandé de liste » mais « tu ne l'as pas déclarée ». La ligne reste (elle fait boucler les totaux), le texte cesse de dédouaner.
  10. **Assertions SQL** : inscription d'autrui refusée ; tournoi `in_progress` refusé (par la fonction *et* par un `update` direct) ; faction hors liste refusée ; `anon` ne lit pas la colonne mais compte toujours les inscrits (l'annuaire public ne doit pas régresser) ; `player_history` renvoie la faction déclarée avec et sans liste d'armée.

  > ✅ **9.3a livrée (2026-08-27)** — migration 0038 appliquée : table de référence `factions` (miroir de `src/lib/factions.ts`, dont le back office aura besoin en US-9.4), colonne `registrations.faction` sous clé étrangère, reprise des factions déjà portées par une liste, `set_registration_faction`, `submit_army_list` qui écrit les deux dans la même transaction, `player_history` qui lit la déclaration. **12 assertions SQL passées** (dont : faction hors référentiel refusée, non-inscrit refusé, comblement accepté après lancement, réécriture refusée par la fonction *et* par un `update` direct, liste validée bloquante, `anon` aveugle sur la colonne mais comptant toujours les inscrits). Deux décisions de sécurité prises au passage : `revoke update` **complet** sur `registrations` (aucun client n'y écrivait directement — tout passe par des fonctions `security definer`), et `select` re-accordé à `anon` colonne par colonne. **Règle permanente : toute colonne ajoutée désormais à `registrations` est privée par défaut.** Conséquence assumée : la faction apparaît dans l'historique d'un tiers, ce que la 0037 masquait — c'est l'arbitrage du 27 août, pas un oubli.
  > ✅ **9.3b livrée (2026-08-27)** — `army-list-card.tsx` devient `preparation-card.tsx` : carte **« Ma préparation »** à deux sous-blocs (Faction, puis Ma liste d'armée inchangée). Enregistrement immédiat via `use-faction-declaration.ts`, **sans `refresh()`** — le hook de la fiche remet `loading` à vrai et l'écran remplacerait tout son corps par un indicateur de chargement à chaque choix. Raccourci « Utiliser ma faction favorite (X) » à un tap, et **aucun préremplissage** : la favorite ne s'enregistre jamais sans un geste. La carte s'affiche aussi après le lancement — et pour un joueur ayant **abandonné**, dont les parties comptent : c'est là qu'on comble.
  > ✅ **9.3c livrée (2026-08-27)** — migration 0039 : `tournament_standings` renvoie `faction` (déclarée) au lieu de `faction_favorite`, et `player_history` la lit du classement au lieu de la recalculer. Côté écrans, la substitution disparaît des **deux applications** : classement mobile et back office, export CSV, modale de clôture, inscrits, pointage, rondes, recherche, et **la faction de l'adversaire dans le bloc « Le jour J »** — le pire endroit où se tromper d'armée. `faction_favorite` a aussi été **retirée des types d'appariement** : la laisser là aurait invité à la réafficher un jour. Elle reste légitime au **profil** et au **roster d'équipe**, qui ne sont pas des contextes de tournoi. Assertion de bout en bout passée : combler après coup remonte jusqu'à l'historique **et** au classement, une seconde écriture est refusée.
  > ⚠️ **Conséquence visible et assumée** : les tournois déjà joués n'ont aucune faction déclarée. Ils n'affichent donc plus rien là où s'affichait la faction favorite, jusqu'à ce que chaque joueur comble depuis la fiche. Mieux vaut ne rien montrer que raconter une histoire fausse (décision 14).
  > **Découpage en trois incréments testables séparément :** **9.3a — Base (S)** critères 1-4 et 10, vérifiable sans écran ; **9.3b — Carte « Ma faction » (M)** critères 5-7, avis `ux-ui` requis ; **9.3c — Cohérence d'affichage (S)** critères 8-9, dans les deux applications.
  > **Arbitrages tranchés :** déclaration *après* l'inscription et jamais pendant (l'inscription en un tap est l'interaction la plus précieuse de l'app ; y greffer un formulaire, c'est payer une donnée de confort avec des inscriptions perdues) ; modifiable jusqu'au lancement, pas jusqu'à la clôture (une faction changée en cours de tournoi falsifierait rétroactivement les tables) ; **facultative et sans repli**, cohérent avec l'absence d'entrée « Autre » — forcer un choix sans échappatoire pousse à déclarer faux, et une déclaration fausse est pire que l'absence.
  > **Sans objet en l'état :** `tournaments` ne porte **aucun drapeau « liste obligatoire »**, la soumission est ouverte à tout inscrit d'un tournoi `open`. La carte est donc proposée uniformément sur tous les tournois. Rendre les listes obligatoires par tournoi serait une autre US, qui ne remettrait pas celle-ci en cause.
  > **Hors périmètre, noté :** interrupteur « masquer ma faction » (ne rien déclarer suffit en v1), stats méta par faction (US-11.3), sous-factions (la liste fermée s'arrête à la faction, l'ouvrir relancerait le débat du texte libre).
- **US-9.4** L'organisateur renseigne ou corrige la faction d'un joueur — **S** — dép. US-9.3. — ✅ **Livrée (2026-08-27)**
  > Migration 0040 : `set_faction_as_organizer(registration_id, faction)`, et le garde-fou de la 0038 apprend à reconnaître l'organisateur. **Deux fenêtres d'écriture, pas une** : le joueur comble mais ne réécrit jamais après le lancement ; l'organisateur corrige tant que le tournoi vit, parce qu'il arbitre et qu'il a l'armée sous les yeux. Après la clôture, tous deux reviennent à la même règle — on comble un vide, on ne retouche pas un classement publié.
  > **Divergence assumée avec l'intitulé** : la cellule vit sur la page **Inscrits**, pas sur Check-in. Les lignes de pointage sont entièrement cliquables (elles basculent la présence) ; y imbriquer une liste déroulante aurait fait cohabiter deux cibles de clic dans le même rectangle, sur l'écran le moins tolérant à l'erreur de toute l'application. La page Inscrits a déjà la colonne Faction et des lignes inertes.
  > `backoffice/src/hooks/use-factions.ts` lit le référentiel `public.factions` posé par la 0038 — **c'est pour ce moment-là qu'il était descendu en base** : le back office n'a aucune copie de `src/lib/factions.ts`. Aucun rechargement après écriture (les corrections vivent dans un état local), même leçon que la saisie des scores.
  > 8 assertions SQL passées, dont : un tiers refusé, faction hors référentiel refusée, correction acceptée en cours de tournoi, **le joueur toujours refusé au même instant**, correction refusée après clôture mais comblement encore accepté.
  > Cas réel : un joueur inscrit la veille qui ne déclare rien ne peut plus jamais combler le trou. C'est utile, mais c'est du back office et une **autre fenêtre d'écriture** — d'où une US séparée plutôt qu'un empilement sur l'US-9.3.
